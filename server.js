...
app.get('/api/health',(req,res)=>{const s=read();res.set('Cache-Control','no-store');res.json({ok:true,version:'3.3.9',storage:'runtime-only',matchdays:Object.keys(s.matchdays).length,counts:Object.fromEntries(Object.entries(s.matchdays).map(([k,p])=>[k,p.matches?.length||0])),details:Object.keys(s.details).length,admin_token_required:!!ADMIN_TOKEN})});
...
app.listen(PORT,()=>{const s=read();console.log(`MagicScanner V3.3.9 listening on port ${PORT}`);console.log(`Store: ${STORE_FILE}`);console.log(`Matchday counts: ${JSON.stringify(Object.fromEntries(Object.entries(s.matchdays).map(([k,p])=>[k,p.matches?.length||0])))}`);console.log(`ADMIN_TOKEN: ${ADMIN_TOKEN?'required':'not configured'}`)});
