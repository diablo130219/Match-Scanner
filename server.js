...
app.get('/api/health',(req,res)=>{const s=read();res.set('Cache-Control','no-store');res.json({ok:true,version:'3.3.2',storage:'runtime-only',matchdays:Object.keys(s.matchdays).length,counts:Object.fromEntries(Object.entries(s.matchdays).map(([k,p])=>[k,p.matches?.length||0])),details:Object.keys(s.details).length,admin_token_required:!!ADMIN_TOKEN})});
...
app.post('/api/matchdays/:date',auth,(req,res)=>{const key=req.params.date;if(!/^\d{4}-\d{2}-\d{2}$/.test(key))return res.status(400).json({ok:false,error:'Data non valida'});const pack=req.body||{};if(!Array.isArray(pack.matches))return res.status(400).json({ok:false,error:'matches deve essere un array'});const s=read();s.matchdays[key]={...pack,date:key,published_at:new Date().toISOString(),matches:pack.matches};write(s);res.json({ok:true,date:key,matches:pack.matches.length,mode:'replace'})});
...
app.listen(PORT,()=>{const s=read();console.log(`MagicScanner V3.3.2 listening on port ${PORT}`);console.log(`Store: ${STORE_FILE}`);console.log(`Matchday counts: ${JSON.stringify(Object.fromEntries(Object.entries(s.matchdays).map(([k,p])=>[k,p.matches?.length||0])))}`);console.log(`ADMIN_TOKEN: ${ADMIN_TOKEN?'required':'not configured'}`)});
